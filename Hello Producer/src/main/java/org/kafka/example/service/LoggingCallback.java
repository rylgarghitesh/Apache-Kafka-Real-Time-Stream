...
public class LoggingCallback implements Callback {
    ...
    public void onCompletion(RecordMetadata recordMetadata, Exception e) {
        if(e !=null){
            ...
            System.out.println("Error sending message string = " + message);
        }else {
            logger.info(recordMetadata + " persisted with offset " + recordMetadata.offset()
                    + " and timestamp on " + new Timestamp(recordMetadata.timestamp()));
            System.out.println(recordMetadata + " persisted with offset " + recordMetadata.offset()
                    + " and timestamp on " + new Timestamp(recordMetadata.timestamp()));
        }
    }
}
